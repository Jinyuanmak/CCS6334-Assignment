<?php
/**
 * Patient addition form for Private Clinic Patient Record System
 * Handles patient registration with encrypted diagnosis storage
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $phoneNumber = Database::sanitizeInput($_POST['phone_number'] ?? '');
    ...
    // Check for duplicate IC number if no validation errors
    if (empty($errors)) {
        try {
            $existingPatient = Database::fetchOne(
                "SELECT id FROM patients WHERE AES_DECRYPT(UNHEX(ic_number), ?) = ?", 
                [SECURE_KEY, $formData['ic_number']], 
                'admin'
            );
            ...
            if ($existingPatient) {
                ...
                Database::logActivity(
                    ...
                    $_SESSION['username'], 
                    'CREATE_FAILED', 
                    "Failed to create patient record: " . $formData['name'] . " - Duplicate IC number: " . $formData['ic_number']
                );
            }
        } catch (Exception $e) {
    ...
    // If no errors, insert the patient record
    if (empty($errors)) {
        try {
            Database::beginTransaction();
            
            $sql = "INSERT INTO patients (name, ic_number, diagnosis, phone) 
                    VALUES (?, HEX(AES_ENCRYPT(?, ?)), AES_ENCRYPT(?, ?), ?)";
            ...
            if ($result > 0) {
                ...
                // Log the patient creation activity for audit trail
                Database::logActivity(
                    $_SESSION['user_id'], 
                    $_SESSION['username'], 
                    'CREATE', 
                    "Created new patient record: " . $formData['name'] . " (IC: " . $formData['ic_number'] . ")"
                );
                ...
                Database::logMessage(
                    'PATIENT_CREATE',
                    'New Patient Added',
                    "New patient " . $formData['name'] . " has been added to the system by administrator.",
                    Database::getLastInsertId()
                );
                
                // Set success message for auto-dismiss alert
                $_SESSION['message'] = 'Patient record added successfully!';
                ...
                Database::rollback();
                $errors['general'] = 'Failed to add patient record. Please try again.';
            }
            
        } catch (Exception $e) {
            Database::rollback();
            error_log("Patient insertion failed: " . $e->getMessage());
            $errors['general'] = 'Database error occurred. Please try again later.';
            
            // Log the failed patient creation attempt
            Database::logActivity(
                $_SESSION['user_id'], 
                $_SESSION['username'], 
                'CREATE_FAILED', 
                "Failed to create patient record: " . $formData['name'] . " - Error: " . $e->getMessage()
            );
        }
    }
}
...
<head>
    ...
    <title>Add New Patient - Private Clinic</title>
    <!-- Bootstrap 5 CSS -->
            ...
            <div class="col-lg-8">
                <div class="card shadow">
                    <div class="card-header bg-success text-white">
                        ...
                        <small>Enter patient information below</small>
                    </div>
                    <div class="card-body">
                ...
                <div class="alert alert-info mt-3">
                    ...
                    <strong>Note:</strong> All fields marked with * are required. Diagnosis information will be encrypted for security and privacy.
                </div>
            </div>
        </div>
    </div>
